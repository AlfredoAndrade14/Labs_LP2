...
class DocumentoRepository {
	...
	public Set<Documento> busca(Map<String, String> metadados) {
		Set<Documento> documentosComOsMetadados = new HashSet<>();

		for (Documento doc : documentos.values()) {
			Map<String, String> metadadosDoc = doc.getMetadados();
			boolean cotemTodosOsMetadados = true;

			for (String metadado : metadados.keySet()) {
				if (metadadosDoc.containsKey(metadado)){
					if (!(metadadosDoc.get(metadado).equals(metadados.get(metadado)))) {
						cotemTodosOsMetadados = false;
					}
				} else { cotemTodosOsMetadados = false; }
			}

			if (cotemTodosOsMetadados) {
				documentosComOsMetadados.add(doc);
			}
		}

		return documentosComOsMetadados;
	}
}
