...
class BuscaRepository {
	 ...
	 *
	 * @param buscaFeita Operação de busca realizada.
	 * @param documentos   Documentos resultantes dessa busca.
	 */
	public void adicionaBusca(Busca buscaFeita, DocumentoDTO[] documentos) {
		String[] ids = Stream.of(documentos)
			.map(DocumentoDTO::getId)
			.collect(Collectors.toList())
			.toArray(new String[] {});
		this.buscas.add(new HistoricoBusca(buscaFeita, ids));
	}
